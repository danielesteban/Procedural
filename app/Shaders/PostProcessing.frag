...
const float innerRadius = 0.40;
const float outerRadius = 0.46;
...
void main(void) {
	...
	if(nightVision != 0.0) {
		...
		if(lum < 0.3) composite *= nightVision * 6.0;
		...
		composite *= smoothstep(outerRadius, innerRadius, distL) + smoothstep(outerRadius, innerRadius, distR);
	}
